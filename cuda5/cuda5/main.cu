...
#define DIR_SIZE 32 //32 bytes per FCB
...
__device__ __managed__ uchar dir[DIR_ENTRIES * DIR_SIZE + 1024];
...
__global__ void mykernel(uchar *input, uchar *output) {
  ...
  fs_init(&fs, volume, SUPERBLOCK_SIZE, FCB_SIZE, FCB_ENTRIES, 
			...
			MAX_FILE_NUM, MAX_FILE_SIZE, FILE_BASE_ADDRESS,dir,DIR_SIZE,DIR_ENTRIES);
